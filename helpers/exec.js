...
export function exec(fn, inputFile, expected, ...args) {
  ...
  if (expected != null && result != expected) console.log('Expected:', expected, '-\x1b[31m FAILED\x1b[0m');
  console.log();
}
...
export function test(fn, input, expected, ...args) {
  ...
  const result = run(fn, input, ...args);
  if (result != expected) console.log('Expected:', expected, '-\x1b[31m FAILED\x1b[0m');
  console.log();
}
